// ==UserScript==
// @name         Download Weibo Images & Videos CC
// @name:zh-CN   下载微博图片和视频 CC
// @version      1.1.7
...
/*27*/  'When \'ZIP mode\' enabled, you have \nto include one of the tags {original} or {index} \nto avoid duplicated ones being overwritten.',
    ...
    function getName(nameSetting, originalName, ext, userName, userId, postId, postUid, index, totalLength, postTime, content, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetContent) {
        let setName = getTemplete(nameSetting, totalLength);
        let padIndex = getPadIndex(index, totalLength);
        ...
        setName = setName.replace('{index}', padIndex);
        ...
        if (retweetPostId && GM_getValue('retweetMode', false)) {
            ...
            let reYYYY, reYY, reMM, reDD, reHH, remm, ress;
            const retweetPostAt = new Date(retweetPostTime);
            if (retweetPostTime) {
                reYYYY = retweetPostAt.getFullYear().toString();
                reYY = retweetPostAt.getFullYear().toString().slice(-2);
            ...
            setName = setName.replace('{re.YY}', reYY);
    ...
    function getTemplete(templet, totalLength) {
        if (totalLength > 1) {
            return templet
        } else {
            return templet.replace("_{index}", "") 
        }
    }

    function getPadIndex(index, totalLength) {
        if (totalLength > 1) {
            return index.toString().padStart(totalLength.toString().length, '0')
        } else {
    ...
    async function handleVideo(mediaInfo, totalLength, userName, userId, postId, postUid, index, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText) {
        ...
        if (isIdMode) {
            const setNameId = getName((GM_getValue('retweetMode', false) && retweetPostId) ? GM_getValue('retweetFileName', '{original}.{ext}') : GM_getValue('dlIdFileName', '{original}.{ext}'), originalName, ext, userName, userId, postId, postUid, index, totalLength, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText);
            newList.push({ url: largeVidUrl, name: setNameId, headerFlag: true });
        }
        if(mediaInfo.hasOwnProperty('pic_info')) {
            ...
            if (isIdMode) {
                const setNameId = getName((GM_getValue('retweetMode', false) && retweetPostId) ? GM_getValue('retweetFileName', '{original}.{ext}') : GM_getValue('dlIdFileName', '{original}.{ext}'), originalName, ext, userName, userId, postId, postUid, index, totalLength, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText);
                newList.push({ url: largePicUrl, name: setNameId, headerFlag: true });
            }
        }
        return newList;
    }

    function handlePic(pic, totalLength, userName, userId, postId, postUid, index, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText) {
        let newList = [];
        ...
        newList.push({ url: largePicUrl, name: setName, headerFlag: true });
        const isIdMode = default_id_mode_list.indexOf(userId) > -1
        console.log("handlePic isIdMode=" + isIdMode);
        if (isIdMode) {
            const setNameId = getName((GM_getValue('retweetMode', false) && retweetPostId) ? GM_getValue('retweetFileName', '{original}.{ext}') : GM_getValue('dlIdFileName', '{original}.{ext}'), originalName, ext, userName, userId, postId, postUid, index, totalLength, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText);
            newList.push({ url: largePicUrl, name: setNameId, headerFlag: true });
        }
            ...
            if(article) {
                ...
                if (picInfos) {
                    ...
                    for (const [id, pic] of Object.entries(picInfos)) {
                        index += 1;
                        downloadList = downloadList.concat(handlePic(pic, totalLength, userName, userId, postId, postUid, index, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText));
                    }
                }
                if (mixMediaInfo && mixMediaInfo.items) {
                    // console.log('mix media');
                    let index = 0;
                    let totalLength = Object.entries(mixMediaInfo.items).length;
                    for (const [id, media] of Object.entries(mixMediaInfo.items)) {
                        ...
                        if(media.type === 'video') {
                            ...
                            downloadList = downloadList.concat(handlePic(media.data, totalLength, userName, userId, postId, postUid, index, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText));
                        }
                    }
                }
                const packName = getName((GM_getValue('retweetMode', false) && retweetPostId) ? GM_getValue('retweetPackFileName', '{mblogid}.zip') : GM_getValue('packFileName', '{mblogid}.zip'), '{original}', '{ext}', userName, userId, postId, postUid, '{index}', 0, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText);
    ...
    function addSingleDlBtn(img, idx = 0) {
        ...
        dlBtn.addEventListener('click', async function(event) {
            ...
            if(article) {
                ...
                if (picInfos) {
                    // console.log('download images');
                    let totalLength = Object.entries(picInfos).length.toString().length;
                    // console.log(idx, picInfos);
                    const pic = Object.entries(picInfos)[idx][1];
                    downloadList = downloadList.concat(handlePic(pic, totalLength, userName, userId, postId, postUid, idx + 1, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText));
                }
                if (mixMediaInfo && mixMediaInfo.items) {
                    // console.log('mix media');
                    // console.log(mixMediaInfo.items);
                    let totalLength = Object.entries(mixMediaInfo.items).length.toString().length;
                    ...
                    if(media.type === 'video') {
                        ...
                        downloadList = downloadList.concat(handlePic(media.data, totalLength, userName, userId, postId, postUid, idx + 1, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText));
                    }
                }
                const packName = getName((GM_getValue('retweetMode', false) && retweetPostId) ? GM_getValue('retweetPackFileName', '{mblogid}.zip') : GM_getValue('packFileName', '{mblogid}.zip'), '{original}', '{ext}', userName, userId, postId, postUid, '{index}', 0, postTime, text, retweetPostId, retweetUserName, retweetUserId, retweetPostUid, retweetPostTime, retweetText);
                handleDownloadList(downloadList, packName);
...
*/
    ...
    function showSetting() {
        ...
        let settingButton = creatButton();
        settingButton.textContent = text[6];
        settingButton.id = 'startButton';
